...
tape( 'the function has an arity of 4', opts, function test( t ) {
	t.strictEqual( srotg.length, 4, 'returns expected value' );
	t.end();
});
...
tape( 'the function computes a Givens plane rotation', opts, function test( t ) {
	...
	for ( i = 0; i < values.length; i++ ) {
		...
		srotg( float64ToFloat32( values[i][0] ), float64ToFloat32( values[i][1] ), out, 1); // eslint-disable-line max-len
...
tape( 'the function returns an array of NaNs if provided a rotation elimination parameter equal to NaN', opts, function test( t ) {
	...
	actual = srotg( NaN, 1.0, new Float32Array( 4 ), 1);
	for ( i = 0; i < actual.length; i++ ) {
		t.strictEqual( isnanf( actual[i] ), true, 'returns expected value' );
	}

	actual = srotg( 1.0, NaN, new Float32Array( 4 ), 1 );
	for ( i = 0; i < actual.length; i++ ) {
		t.strictEqual( isnanf( actual[i] ), true, 'returns expected value' );
	}

	actual = srotg( NaN, NaN, new Float32Array( 4 ), 1 );
	for ( i = 0; i < actual.length; i++ ) {
		t.strictEqual( isnanf( actual[i] ), true, 'returns expected value' );
	}
	t.end();
