...
Returns the logarithm of the probability density function (PDF) for a logistic distribution with location parameter `mu` and scale parameter `s` at a value `x`.
...
int main( void ) {
    ...
    for ( i = 0; i < 25; i++ ) {
        ...
        printf( "x: %lf, µ: %lf, s: %lf, Var(X;µ,s): %lf\n", x, mu, s, y );
    }
}
```
