...
tape( 'the function correctly identifies packages with C implementation', function test( t ) {
	...
	// abs is a simple package that has C implementation
...
tape( 'the function correctly identifies JS-only packages', function test( t ) {
	var result = cImplStatus( '@stdlib/math/base/special/sici' );
	// sici is JS-only (no src/)
	t.equal( result.pkgStatus.hasSrc, false, 'sici has no src/' );
	t.equal( result.pkgStatus.hasC, false, 'sici has no C' );
	t.equal( result.pkgStatus.isJSOnly, true, 'sici is marked as JS-only' );
	t.end();
});

tape( 'the function handles dev dependencies option', function test( t ) {
	var result1 = cImplStatus( '@stdlib/math/base/special/beta', { 'dev': false } );
	var result2 = cImplStatus( '@stdlib/math/base/special/beta', { 'dev': true } );
